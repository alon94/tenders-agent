...
export const PUBLISHERS: PublisherDef[] = [
  ...
  { id: "local", label: "רשויות מקומיות", kw: ["עיריית", "עירייה", "עיריה", "מועצה", "מועצת", "רשות מקומית", "אזורית", "מ.מ ", "מ.א ", "מעלה אדומים", "ירושלים", "תל אביב", "חיפה", "באר שבע", "ראשון לציון", "פתח תקווה", "אשדוד", "נתניה", "חולון", "בני ברק", "רמת גן", "אשקלון", "רחובות", "בת ים", "הרצליה", "כפר סבא", "מודיעין", "רעננה", "לוד", "רמלה", "נצרת", "עכו", "אילת", "טבריה", "צפת", "קריית", "גבעתיים", "הוד השרון", "ראש העין", "נס ציונה", "יבנה", "דימונה", "אופקים", "שדרות", "נהריה", "כרמיאל", "עפולה", "בית שמש", "ביתר", "אריאל", "איגוד ערים", "חברה כלכלית", "החברה העירונית", "תאגיד המים", "משכ\"ל", "החברה למשק וכלכלה"] },
...
export function matchPublisher(t: ClassifiableTender, pubId: string): boolean {
  ...
  if (!p) return true;
  const pub = ((t.publisher || "") + " " + (t.type || "")).toLowerCase();
  // QA #01: רשומות מוניציפליות מזוהות גם לפי מזהה (muni-*) — גם כשהשם הוא שם עיר חשוף.
  if (pubId === "local" && (t.id || "").startsWith("muni-")) return true;
