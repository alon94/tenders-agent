...
export default function Dashboard(){
  ...
  const load=useCallback(async()=>{
    ...
    try{
      ...
      setAll(arr);
      fetch('/api/tenders?count=1').then((r) => r.json()).then((c) => setTotal(typeof c.total === 'number' ? c.total : null)).catch(() => {});
  ...
  const kpis=[
    {value: total ?? all.length,label:'מכרזים פעילים במאגר',dot:BLUE},
